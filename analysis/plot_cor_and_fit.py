...
# indir = "/global/cfs/cdirs/desi/science/lya/picca_on_mocks/saclay/v4.7/global-11_20/eboss-0.0"
indir = "/global/cfs/cdirs/desi/science/lya/picca_on_mocks/saclay/v4.7/v4.7.01/eboss-0.0"
# patern = "xcf_z_{zmin}_{zmax}-exp.{ext}"
patern = "xcf_z_{zmin}_{zmax}-exp.{ext}"
patern_cf = "xcf_z_{zmin}_{zmax}-exp.{ext}"
...
for i in range(len(z_bins)-1):
    filename_fits = indir+"/"+patern_cf.format(zmin=z_bins[i], zmax=z_bins[i+1], ext='fits')
    data = fitsio.FITS(filename_fits)
    da = data[1].read()['DA']
    co = data[1].read()['CO']
    data.close()
    filename_h5 = indir+"/"+patern.format(zmin=z_bins[i], zmax=z_bins[i+1], ext='h5')
    ...
    for j in range(len(mu_bins)-1):
        ...
        axs[i//2, i%2].errorbar(wedge_data[0],coef*wedge_data[1],yerr=coef*np.sqrt(np.diag(wedge_data[2])),fmt='+', label=r"${}<\mu<{}$".format(mu_bins[j], mu_bins[j+1]), color=colors[j])
