...
def main():
    ...
    P_camb = PowerSpectrum.P_0("data/PlanckDR12.fits")
    Pcamb = P_camb.P(kk)
    kny = PI/DX
    k = np.arange(kny/dk)*dk
    #k = np.arange(PI/0.2/dk)*dk  # prov
    ...
    if RSD:
        ...
        PRSD = P_RSD(k_par_t,k_perp,P_camb.P)
        P1DcambRSD = PowerSpectrum.P_1D_RSD(k_par,k_perp,PRSD).P1D(k)
    ...
    #     P1DWcamb = PowerSpectrum.P_1D(kk,Pcamb*W*W).P1D(k)
    #     plt.plot(k,P1DWcamb,color="blue")
        
    #     if (RSD) :
    #         PRSD = P_RSD(k_par_t,k_perp,PW2)
    #         P1DWcambRSD = PowerSpectrum.P_1D_RSD(k_par,k_perp,PRSD).P1D(k)
    #         plt.plot(k,P1DWcambRSD,color="black")
    #     print ("1")
    ...
    # Pcamb_cut=Pcamb[cut]
    # if (False):
    #     P1Dcutcamb = PowerSpectrum.P_1D(kk_cut,Pcamb_cut).P1D(k)
    #     plt.plot(k,P1Dcutcamb,color="blue")
    #     #plt.plot(k,P1Dcutcamb,color="green")
        
    #     if (RSD) :
    #         PRSD = P_RSD(k_par_t,k_perp,Pcut)
    #         P1DcutcambRSD = PowerSpectrum.P_1D_RSD(k_par,k_perp,PRSD).P1D(k)
    #         plt.plot(k,P1DcutcambRSD,color="green")
    #     print ("2")
    ...
    P1DWcutcamb = PowerSpectrum.P_1D(kk_cut,Pcamb_cut*W*W).P1D(k)

    if RSD:
        PRSD = P_RSD(k_par_t,k_perp,PW2cut)
        P1DWcutcambRSD = PowerSpectrum.P_1D_RSD(k_par,k_perp,PRSD).P1D(k)

    #.................................      missing P^1D(k)
    P1Dmissing = np.maximum(interpolate.InterpolatedUnivariateSpline(k,P1Dcamb - P1DWcutcamb),0)  

    if RSD:
        P1DmissingRSD = np.maximum(interpolate.InterpolatedUnivariateSpline(k,P1DcambRSD - P1DWcutcambRSD),0) 

    # Write to fits file
    print("P1D computed. Writting file...")
    outfits = fitsio.FITS(outfile, 'rw', clobber=True)
    table = [k, P1Dmissing(k)]
    names = ['k', 'P1Dmiss']
    if RSD:
        table.append(P1DmissingRSD(k))
    ...
    if do_plots:
        print("plotting...")
        f, ax = plt.subplots()
        ax.plot(k, P1Dcamb, color="blue", label='Pcamb')
        ax.plot(k, P1DcambRSD, color="green", label='PcambRSD')
        ax.plot(k, P1DWcutcamb, color="blue", label='Pcamb_Wcut')
        ax.plot(k, P1DWcutcambRSD, color="green", label='PcambRSD_Wcut')
        ax.plot(k, P1Dmissing(k), color="red", label='Pmissing')
        ax.plot(k, P1DmissingRSD(k), color="red", label='PmissingRSD')
